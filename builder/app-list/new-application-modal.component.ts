...
export class NewApplicationModalComponent implements OnInit {
    ...
    constructor(public bsModalRef: BsModalRef, private appService: ApplicationService, private appStateService: AppStateService,
        ...
        private settingsService: SettingsService, private appListService: AppListService, private modalService: BsModalService) { }
    ...
    async createApplication() {
        ...
        if (this.isImportApp) {
            let desc = "You are about to import application " + this.appName + ". Please note that devices, widgets, and smart rules are not supported. You may require to configure those manually." + "\n" +" Do you want to proceed ?";
            const alertMessage = {
            ...
            const importAppDialogRef = this.alertModalDialog(alertMessage);
            importAppDialogRef.content.event.subscribe(async data => {
    ...
    async importApplication() {
        ...
        await Promise.all(appDashboards.map(async dashboard => {
            await this.addTemplateDashboard(this.fileData, dashboard.name, dashboard.icon, dashboard.c8y_Dashboard, (dashboard.deviceId ? dashboard.deviceId : ''), dashboard.id, dashboard.groupTemplate);
        }));
        let defaultAppBuilderData: any;
        defaultAppBuilderData = {
            applicationBuilder: {
                ...
                selectedTheme: this.fileData.selectedTheme
            },
            icon: {
