...
@Component({
    ...
    <div class="modal-body c8y-wizard-form">
        <form name="editAnalyticsProviderForm" #editAnalyticsProviderForm="ngForm" class="c8y-wizard-form">
    ...
    <div class="modal-footer">
        ...
        <button type="button" class="btn btn-primary" [disabled]="!editAnalyticsProviderForm.form.valid" (click)="updateAnalyticsProvider()">Save</button>
    </div>
  `
})
...
export class EditAnalyticsProviderModalComponent {
    ...
    async updateAnalyticsProvider() {
        ...
        creationAlert.update('Updating Provider...');
            if(AppBuilderConfig) {
            const analyticsProviderList = AppBuilderConfig.analyticsProvider.filter( provider => provider.id !== this.analyticsProvider.id);
